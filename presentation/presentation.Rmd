...
## One portfolio from Modern Portfolio Theory (MPT)

One portfolio from MPT (@markowitz1959portfolio) is the Global Minimum Variance (GMV) portfolio, given by
$$
...
n<-100
tibble(
  "GMV" = c(1, cumprod(1+rnorm(n,0.003, 0.02))),
  "Not GMV" = c(1,cumprod(1+rnorm(n, 0.01, 0.06))),
  "x"=0:n
) %>% 
  pivot_longer(-x) %>% 
  ggplot() +
...
## Not knowing what to own & diversification
...
- We want $p$ to be large, e.g _diversify_ a lot.
  ...
  * We want to use $\bS^{-1}$ for GMV, not $\bS$. How accurate is $\bS^{-1}$ or
  ...
  $$
- Estimates are ''uncertain'' $\rightarrow$ can we understand that uncertainty or construct ''better'' estimates?
...
$$
\widehat{\bw}_{SH}=\psi \widehat{\bw}_{GMV} + (1-\psi)\bb
$$
where $\bb$ is "some portfolio".

- Introduces bias to the portfolio but will lower its variance.
- How do we determine $\psi$? $$\min_{\psi} \widehat{\bw}_{SH}^\top \bSigma \widehat{\bw}_{SH}$$
_note that $\bSigma$ is present again!_

## Paper IV - Is variance informative?

```{r, warning=FALSE, fig.height=2.5, fig.align='center', fig.pos='top', fig.}
offset <- 0.01
tibble(x = 1:11,
       y = 1) %>% 
  ggplot() +
...
- assume little, construct ''good'' portfolios but we might understand less.
