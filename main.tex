...
%%%%%% ------------------------------------------------------------------------
\chapter[Estimation \& models]{Statistical models and inference}\label{ch:estim}
%%%%%% ------------------------------------------------------------------------
...
%Assume that $\bY \sim N_{p,n}(\bmu \ones_n^\top, \bSigma \otimes \bI_n)$ then $\optn{Var}(\byb)=\bSigma / n$ but $\optn{Var}(\bS)=$
...
\begin{equation}\label{eqn:meanvar_characteristics_sample}
  \hV = \frac{1}{\ones^\top \bS^{-1}\ones},\; \hR = \frac{1}{\ones^\top \bS^{-1}\ones}
\end{equation}
...
$$
\frac{\bS^{-1}\ones}{\ones\bS^{-1}\ones}
$$
contain the sample covariance matrix in the nominator as well as the demoniator.
Furthermore, the sample covariance matrix is everywhere so its not safe to assume that these are independent.
...
The following theorem is used immensely in papers 1 and 2.
...
\textbf{Give an example from this setup and how to derive the distribution of different quantitites.}

Paper 1 derives the full joint distribution for all the quantities used in \eqref{eqn:meanvar_solution_sample}.
