...
<<mertons_efficient_frontier, fig.cap="Efficient frontiers with and without a risk-free asset. The left plot illustrates two different efficient frontiers for different portfolio sizes. The right plot we illustrate the efficient frontier and the capital market line which appears when a riskless asset is available. The stocks are randomly selected from the S\\&P500. The individual means and standard deviations are displayed as points.", fig.height=3.5>>=
set.seed(4)
...
ggplot_df <- map_dfr(x, ~{
  ...
  mutate(type="efficient frontier",
         "Riskless asset avaiable"="No riskless asset avaiable")
...
ggplot_df <- tibble("V"=V,"R"=R, 
                        ...
                        "portfolio size"="30",
                        "Riskless asset avaiable"="Riskless asset avaiable") %>%
  bind_rows({
    map_dfr(gamma, ~{
...
ggplot_df %>%
  ...
  theme_minimal() +
  facet_grid(.~`Riskless asset avaiable`) +
