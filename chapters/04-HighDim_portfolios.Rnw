...
Below is a small motivating example to why using all data at once might be desirable.
...
In this example we omit the code, but it is available on github.
<<cv_benchmark, cache=TRUE, echo=1:30, fig.cap="Out of sample variance estimates from the 5-fold cross validation.", fig.height=4>>=
p <- 150
n <- 250
K <- 5
b <- rep(1,p) / p # use EW portfolio as target 
...
@
From figure \ref{fig:cv_benchmark} we can see that the optimal value is somewhere around $0.5$ according to cross validation. 
As it turns out, cross validation suggest that the optimal value should be $\Sexpr{min_vals$loss}$. 
The analytic method from \citet{bodnar2018estimation} suggest that the optimal value is $\Sexpr{round(w_bodnar2018$alpha,4)}$.
The "correct" value of $\alpha$, as given by the oracle estimator, is equal to $\Sexpr{round(alpha_oracle, 4)}$. 
Its limiting value (see Theorem 2.1 \citet{bodnar2018estimation}) is equal to $\Sexpr{round(alpha_true, 4)}$.
Although none of the methods are spot on, the bona fide estimator is closer to what the optimal value should be.
Furthermore, changing the number of folds in the code above can give quite different results in what the optimal value should be.
