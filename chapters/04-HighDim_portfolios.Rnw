...
<<cv_bench, cache=TRUE, echo=TRUE, ref="high-dim-CV", codecap="R-code performing a 5-fold cross validation for determining shrinkage coefficients as well as the implementation from the HDShOP package.">>=
# setup
set.seed(123)
p <- 150
n <- 250
...
# perform 5-fold CV, pmap to map over rows 
result <- pmap(grid, ~{
    test <- Y[,folds[[.y]]]
    train <- Y[,-folds[[.y]]]
    S <- var(t(train))
