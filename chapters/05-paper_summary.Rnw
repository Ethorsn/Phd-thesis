...
\section*{Paper 3}
This paper deals with the fact that taking limits changes estimates. If the investor invest in a portfolio and then wait for a wekk, month or year their perception of what portfolio they should hold will have changed. This is implicit when taking limits.  One can therefore ask how one can rebalance optimally when new data arrives. Assuming that the investor is ok with rebalancing the portfolio at fixed time points we develop a reweighting scheme to do so optimally. Assuming that the investor is completely risk-averse, the proposed method is optimal and consistent with the investors preference. The reallocation is done to be optimal in minimizing the out-of-sample variance. One nice feature of this scheme is that it also decreases transaction costs.
